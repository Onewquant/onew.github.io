...
가상 환경은 보통 프로젝트 폴더 안에 venv라는 폴더로 되어있다. 그림으로 보면 아래와 같다. 설명을 위해 'TestPack'이라는 패키지를 만들었다고 가정하겠다. 여기서 중요한 것은 패키지 폴더 내에 ```__init.py__``` 파일이 반드시 있어야 패키지로 인식된다는 점이다. 따라서 만약 이 파일을 안 만들어 놓으면 패키지 폴더가 포함 안 된 채 빈 껍데기 패키지만 빌드업 된다. 파일 속 내용은 비어있더라도 ```__init__.py``` 파일 자체는 꼭 만들자 ! 
